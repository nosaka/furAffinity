...
import ns.me.ns.furaffinity.databinding.ListItemGalleryContentsBinding
 ...
 * Gallery Adapter
 */
class GalleryAdapter(context: Context) : AbstractRecyclerViewAdapter<GalleryAdapter.ContentsViewModel>(context) {
     ...
     * データコンテンツViewModel
     */
    class ContentsViewModel(value: Gallery) : ViewInterface {
    ...
    class ContentsViewHolder(itemView: View, cellHeight: Int?) : AbstractRecyclerViewAdapter.ViewHolder(itemView) {
        val binding: ListItemGalleryContentsBinding = DataBindingUtil.bind(itemView)
    ...
    override fun createDataViewHolder(inflater: LayoutInflater, parent: ViewGroup, dataViewType: Int): AbstractRecyclerViewAdapter.ViewHolder? =
            when (dataViewType) {
                TYPE_DATA_CONTENTS -> ContentsViewHolder(inflater.inflate(R.layout.list_item_gallery_contents, parent, false), cellHeight)
                else -> null
            }
